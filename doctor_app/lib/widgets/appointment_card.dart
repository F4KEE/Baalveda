    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not launch system dialer'), backgroundColor: AppTheme.primaryNavy),
        );
      }
    } finally {
        ...
        child: Column(
          ...
          children: [
                ...
                Container(
                  ...
                  child: Center(
                    child: Text(
                      ...
                      style: const TextStyle(color: AppTheme.primaryNavy, fontSize: 16, fontWeight: FontWeight.w700),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        patientName,
                        style: Theme.of(context).textTheme.titleLarge?.copyWith(fontSize: 16, color: AppTheme.primaryNavy),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                      const SizedBox(height: 2),
                      Text(
                        'Pediatric & Neonatology Care',
                        style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppTheme.textLight, fontSize: 12),
                      ),
                    ],
                  ),
                ),
            ...
            Row(
              children: [
                Expanded(
                  child: _DetailRow(icon: Icons.access_time_rounded, label: 'Time', value: appointmentTime, color: AppTheme.accentRose),
                ),
                Expanded(
                  child: _DetailRow(icon: Icons.calendar_today_rounded, label: 'Date', value: formattedDate, color: AppTheme.primaryNavy),
                ),
              ],
            ),
            ...
            Row(
              children: [
                Expanded(
                  child: _DetailRow(icon: Icons.person_outline_rounded, label: 'Doctor', value: 'Dr. $doctor', color: AppTheme.textMedium),
                ),
                Expanded(
                  child: _DetailRow(icon: Icons.phone_enabled_outlined, label: 'Phone', value: phone, color: AppTheme.textMedium),
                ),
              ],
            ),
            const SizedBox(height: AppTheme.spacing16),

            // Clean "Call Patient" button action
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                label: Text(_isCalling ? 'Calling...' : 'Call Patient'), // Replaced textual labels
